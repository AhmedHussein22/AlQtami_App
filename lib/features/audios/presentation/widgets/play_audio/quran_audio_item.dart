...
class AudioMediaCard extends StatefulWidget {
  ...
  const AudioMediaCard(
      {super.key,
      required this.index,
      required this.allQuran,
      this.isFromDonloads = false,
      this.audioPath});
...
class AudioMediaCardState extends State<AudioMediaCard> {
  ...
  void shareSurah(String audioId) async {
    ...
    Share.share(
        'استمع الان الي ناصر القطامى سورة ${widget.allQuran.titleAr} : $dynamicLink');
  }
        ...
        child: ListTile(
          ...
          trailing: Row(
            ...
            children: [
              ...
              0.02.sw.horizontalSpace,
              BlocBuilder<DownloadMediaCubit, DownloadMediaState>(
                  builder: (context, state) {
                return InkWell(
                  onTap: widget.isFromDonloads
                      ? () async {
                          final downloadSurListPaths =
                              await Preference.getStringList(
                                      PrefKeys.downloadSurListPaths) ??
                                  {};
                          bool removeDone =
                              downloadSurListPaths.remove(widget.audioPath);
                          if (removeDone) {
                            Preference.setStringList(
                                PrefKeys.downloadSurListPaths,
                                downloadSurListPaths.toList());
                      ...
                      : () async {
                          ...
                          final download =
                              BlocProvider.of<DownloadMediaCubit>(context);
                          await download.downloadMedia(
                              fileName: '${widget.allQuran.index}.mp3',
                              path:
                                  "${EndPoints.quranAudio + widget.allQuran.index}.mp3");
                          if (state is DownloadMediaComplete) {
                            Directory appDocDirectory =
                                await getApplicationDocumentsDirectory();
                            final String filePath =
                                '${appDocDirectory.path}/${widget.allQuran.index}.mp3';
                            Logger().d("pathhhhhhhhhhhhhhhhhhh0000 $filePath");

                            final downloadSurPaths =
                                await Preference.getStringList(
                                        PrefKeys.downloadSurListPaths) ??
                                    {};
                            Logger().d(
                                "pathhhhhhhhhhhhhhhhhhh0000 $downloadSurPaths");

                            if (!downloadSurPaths.contains(filePath)) {
                              downloadSurPaths.add(filePath);
                              Preference.setStringList(
                                  PrefKeys.downloadSurListPaths,
                                  downloadSurPaths.toList());
                              UIGlobal.showToast(
                                  msg: tr(context, 'completedDownload'));
                            } else {
                              UIGlobal.showToast(
                                  msg: tr(context, "alreadyDownload"));
                            }
                          } else if (state is DownloadMediaError) {
                            UIGlobal.showToast(
                                msg: tr(context, 'fildDownload') +
                                    state.errormsg);
                          }
                        },
                  child: Icon(
                      widget.isFromDonloads
