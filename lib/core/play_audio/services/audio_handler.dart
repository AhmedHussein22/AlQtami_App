...
Future<AudioHandler> initAudioService() async {
  return await AudioService.init(
    ...
    config: const AudioServiceConfig(
      androidNotificationChannelId: 'com.MRcode.Naser_Alqtami',
      androidNotificationChannelName: 'AlQatami',
      androidNotificationOngoing: true,
      androidStopForegroundOnPause: true,
      androidShowNotificationBadge: true,
      notificationColor: AppColors.DARK_BLUE_COLOR,
    ),
  );
}
...
class MyAudioHandler extends BaseAudioHandler {
  ...
  AudioSource _createAudioSource(MediaItem mediaItem) {
    final String path = mediaItem.extras!['path'] ?? "";
    Set<String> downloadPaths = {};
    Preference.getStringList(PrefKeys.downloadSurListPaths).then((value) {
      //Logger().d(value);
      downloadPaths = value ?? {};
    });
    if (downloadPaths.contains(path)) {
      Logger().d(downloadPaths);
